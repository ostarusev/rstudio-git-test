...
install.packages(c("tidyverse", "lubridate", "timeDate"))
...
time_series <- data.frame(Date = as.timeDate(dates), Value = values)

# Преобразование класса Date в timeDate
time_series$Date <- as.timeDate(time_series$Date)
...
time_series <- time_series %>%
  mutate(Weekday = wday(Date, label = TRUE),
         ...
         DayofMonth = day(Date),
         Holiday = as.integer(!isBizday(Date)))

# Конвертация timeDate обратно в Date для ggplot2
